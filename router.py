...
from database.database import read_detail_movie,delete_comment,get_comments,create_comment,read_all_favorite_movie,movie_to_favorite, read_all_genres, read_all_movie, create_user_db, get_auth_user, b64crypt_decode, get_favorite
from database.serializers import UserRegistrationSerializer, UserLoginSerializer
from request_body import MovieRequest,CommentRequest
...
async def detail_movie(pk):
    movie = read_detail_movie(pk=pk)
    return movie

@router.get("/movie/")
async def movie():
    movie = read_all_movie(page=1)
    return movie
...
async def movie(movie_data:MovieRequest):
    ...
    string = movie_data.string 
    genres = movie_data.genres
    movie = read_all_movie(page=page,string=string,genres=genres)
    return movie